...
app.post('/api/tasks', (req, res) => {
  const { id, id_user, id_group, num, name, bg, color, description, disabled, visible } = req.body
  ...
  ensureTableExists(tableTasks, (error) => {
    ...
    if (id > 0) {
      const updateQuery = `
        UPDATE eventtasks
        SET id_user = ?, id_group = ?, num = ?, name = ?, bg = ?, color = ?, description = ?, disabled = ?, visible = ?
        WHERE id = ?
      `
      connection.query(
        updateQuery,
      ...
      const insertQuery = `
        INSERT INTO eventtasks (id_user, id_group, num, name, bg, color, description, disabled, visible)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `
      connection.query(
        insertQuery,
...
app.post('/api/groups', (req, res) => {
  const { id, id_user, num, name, description, disabled, visible } = req.body
  ...
  ensureTableExists(tableGroups, (error) => {
    ...
    if (id > 0) {
      const updateQuery = `
        UPDATE eventgroups
        SET id_user = ?, num = ?, name = ?, description = ?, disabled = ?, visible = ?
        WHERE id = ?
      `
      connection.query(
        updateQuery,
        [id_user, num, name, description, disabled, visible, id],
        (updateError, results) => {
          if (updateError) {
            console.error('Chyba při aktualizaci databáze:', updateError)
            res.status(500).send('Chyba na serveru při aktualizaci záznamu')
      ...
      const insertQuery = `
        INSERT INTO eventgroups (id_user, num, name, description, disabled, visible)
        VALUES (?, ?, ?, ?, ?, ?)
      `
      connection.query(
        insertQuery,
